...
const { Option } = Select;
export default function Dam_Captioning() {
    let Newarr:any[] = [];
  ...
  const [dam_type_name, setDamTypeName]=useState<string>("")
    ...
    for(let i = 1; i < typeArr.length+1; i++){
        images.push({ 
            num:i, 
            folderName:typeArr[i-1], 
            imageURL: [],
            cnt:0,
            })
   ...
   const onChangeDamType = (e:any) => {
        ...
        setImgList(copy)
        setModel('BEST_checkpoint_dam_5_cap_per_img_5_min_word_freq.pth.tar')
        setWord('WORDMAP_dam_5_cap_per_img_5_min_word_freq.json')
        setCls('Dam_model.pt')
