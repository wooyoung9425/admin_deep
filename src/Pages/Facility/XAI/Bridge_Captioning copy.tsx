...
export default function Bridge_Captioning() {
   ...
   const onChangeBridgeType = (e:any) => {
    ...
    const onChangeBridgeOption = (e: any) => {
        ...
        if (BridgeType === 'Girder') {
            for (let i = 0; i < optionCount[0]; i++){
                copy[i] = { ...copy[i], num: e, folderName:'G'+String(i+1).padStart(2,'0') }
                Newarr[i] = { folder: BridgeType+'/'+e+'/G'+String(i+1).padStart(2,'0'), filename: [] };
                axios({
                    method: 'get',
                    url: API_URL+'/File/Files',
                    headers: { "accept": `application/json`, "access-token": `${token}` },
                    params: {
                        path: `/project/${project_id}/stage0/${BridgeType + '/' + e}/${'G' + String(i + 1).padStart(2, '0')}`
                    }
                }).then((res)=>{
                    if (res.data.check === true) {
                        console.log("성공")
                        ...
                        for (let j = 0; j < res.data.data.files.length; j++){
                            imgURL.push(`${IMAGE_URL}/image?path=/project/${project_id}/stage0/${BridgeType + '/' + e}/${'G' + String(i + 1).padStart(2, '0')}/${res.data.data.files[j]}&width=360`)
                            setImgList(copy)
                        }
            ...
            const f_name: string = 'S' + e.substring(2, 4)
            copy = []
            copyCNT = []
            for (let i = 0; i < optionCount[3]; i++) {
                copy.push({ num: e, folderName: f_name + String(i + 1).padStart(2, '0'), cnt: 0, imageURL: [] })
                Newarr[i] = { folder: BridgeType + '/' + e + '/' + f_name + String(i + 1).padStart(2, '0'), filename: [] };
                ...
                axios({
                    ...
                    headers: { "accept": `application/json`, "access-token": `${token}` },
                    params: {
                        path: `/project/${project_id}/stage2/${BridgeType + '/' + e}/${f_name+ String(i + 1).padStart(2, '0')}`
                    }
                }).then((res) => {
                    if (res.data.check === true) {
                        console.log("성공")
                        ...
                        setResult(true);
                        for (let j = 0; j < res.data.data.files.length; j++) {
                            imgURL.push(`${IMAGE_URL}/image?path=/project/${project_id}/stage2/${BridgeType + '/' + e}/${f_name + String(i + 1).padStart(2, '0')}/${res.data.data.files[j]}&width=360`)
                            setImgList(copy)
                        }
                        copy[i] = { ...copy[i], imageURL: imgURL }
                    } else {
            ...
            for (let i = 0; i < optionCount[2]; i++){
                copy.push({num: e, folderName: 'Slab' + String(i + 1).padStart(2, '0'),cnt:0, imageURL:[]})
                Newarr[i] = { folder: BridgeType + '/' + e + '/Slab' + String(i + 1).padStart(2, '0'), filename: [] };
                axios({
                    method: 'get',
                    url: API_URL + '/File/Files',
                    headers: { "accept": `application/json`, "access-token": `${token}` },
                    params: {
                        path: `/project/${project_id}/stage0/${BridgeType + '/' + e}/${'Slab' + String(i + 1).padStart(2, '0')}`
                    }
                }).then((res) => {
                    if (res.data.check === true) {
                        console.log("성공")
                        ...
                        setResult(true);
                        for (let j = 0; j < res.data.data.files.length; j++) {
                            imgURL.push(`${IMAGE_URL}/image?path=/project/${project_id}/stage0/${BridgeType + '/' + e}/${'Slab' + String(i + 1).padStart(2, '0')}/${res.data.data.files[j]}&width=360`)
                            setImgList(copy)
                        }
                        copy[i] = { ...copy[i], imageURL: imgURL }
                    } else {
            ...
            for (let i = 0; i < optionCount[1]; i++) {
                copy.push({ num: e, folderName: 'S' + String(i + 1).padStart(3, '0'), cnt: 0, imageURL: [] })
                Newarr[i] = { folder: BridgeType + '/' + e + '/S' + String(i + 1).padStart(3, '0'), filename: [] }
                copyCNT.push({ camera_number:e, cnt:0})
                ...
                axios({
                    ...
                    headers: { "accept": `application/json`, "access-token": `${token}` },
                    params: {
                    path: `/project/${project_id}/stage2/${BridgeType+'/'+e + '/S' + String(i + 1).padStart(3, '0')}`
                    }
                }).then((res) => {
                    if (res.data.check === true) {
                        console.log("성공")
                        ...
                        let imgURL = [];
                        for (let j = 0; j < res.data.data.files.length; j++) {
                            imgURL.push(`${IMAGE_URL}/image?path=/project/${project_id}/stage2/${BridgeType+'/'+e+ '/S' + String(i + 1).padStart(3, '0')}/${res.data.data.files[j]}&width=360`)
                        }
                        copy[i] = { ...copy[i], imageURL: imgURL }
                    } else {
                        console.log("실패")
