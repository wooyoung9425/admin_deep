...
function ProjectDashboardEstimator() {
    ...
    if (project_Type === 'Tunnel') {
        task_name=["crack_drawer", "estimator"]
    } else if (project_Type === 'Airport') {
        task_name=['crack_defect_merge','defect_estimator','crack_drawer_step1']
    } else if (project_Type === 'Dam') {
        task_name=['drawer_dam', 'estimator_dam']
    } else if (project_Type === 'Bridge') {
        task_name = ['drawer_bridge','countcsv','estimator_bridge']
    }
    ...
    const response1 = axios({
        ...
        if (res.data.check === true) {
            ...
            for (let i = 0; i < res.data.data.jobs.length; i++) {
                // const crack_drawer=["crack_drawer", "estimator"]
                    ...
                    axios({
                        ...
                        if (res2.data.check === true) {
                            ...
                            Object.values(res2.data.data.tasks).map((ttt, index) => {
                                j = j + 1
                                // console.log(Object(ttt).task_id)
...
export default ProjectDashboardEstimator;
