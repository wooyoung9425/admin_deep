...
function PanoramaDashboardImageEdit() {
    ...
    const response1 = axios({
        ...
        if (res.data.check === true) {
            ...
            for (let i = 0; i < res.data.data.jobs.length; i++) {
                const panorama = ["hor_stitch","create_csv","cut_image"]
                if (panorama.includes(res.data.data.jobs[i].task_name)===true) {
                    axios({
                        method: "post",
...
export default PanoramaDashboardImageEdit;
