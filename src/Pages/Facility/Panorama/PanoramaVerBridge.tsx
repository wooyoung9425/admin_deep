                ...
                if(BridgeType === "Girder"){          
                    for(let i = 0; i < settings.span_count; i++){
                        axios({
                            ...
                            headers: { "accept": `application/json`, "access-token": `${token}` },
                            params: {
                                path: `/project/${project_id}/stage5/${BridgeType}/P${"0"+settings.span_number_list[i].num}`
                            }
                            }).then((res)=>{
                                for(let y = 0; y < res.data.data.files.length; y++){
                                    ...
                                    OriImage.push({
                                        original_image : `${IMAGE_URL}/image?path=/project/${project_id}/stage5/${BridgeType}/P${"0"+settings.span_number_list[i].num}/${res.data.data.files[y]}&width=1440`,
                                        no : OriImage.length
                                    })
                ...
                if(BridgeType === "Pier"){
                    for(let i = 0; i < settings.pier_count; i++){
                        ...
                        axios({
                            ...
                            headers: { "accept": `application/json`, "access-token": `${token}` },
                            params: {
                                path: `/project/${project_id}/stage5/${BridgeType}/P0${settings.pier_number_list[i].num}`
                            }
                            }).then((res)=>{
                                for(let y = 0; y < res.data.data.files.length; y++){
                                    const indexFront = res.data.data.files[y].indexOf("S") 
                                    ...
                                    OriImage.push({
                                        original_image : `${IMAGE_URL}/image?path=/project/${project_id}/stage5/${BridgeType}/P0${settings.pier_number_list[i].num}/${res.data.data.files[y]}&width=1440`,
                                        no : OriImage.length
                                    })
    ...
    const option_render=()=>{
        const arr: any[] = [];
            typeArr.map((type: any) => {
                ...
                arr.push(<Option value={type} disabled={type === 'GirderSide' ? true : false}> {name}</Option>)
            })
        return arr;
    }
            ...
            result.push(
                <div className={styles.PanoramaDiv} >                    
                    ...
                    <div className={styles.PanoramaImage}>
                        <Image src={resultOri === true ? ImgListOri[num].original_image : ""} alt="" width={1080}/>
                    </div>
                </div>
            );
    ...
    return (
        <>
            <div className={styles.DivArea}>
                <div>
                    <p className={styles.subOrder}>{t.PdPanorama} &gt; {t.pnHorizontalPanorama}</p>
