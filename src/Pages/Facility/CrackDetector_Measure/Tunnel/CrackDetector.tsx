...
export default function CrackDetector() {
                    ...
                    axios({
                        ...
                        if (res.data.check === true) {
                            ...
                            for(let i = 0; i<res.data.data.files.length; i++){
                                ...
                                // console.log(SpanNum)
                                ResultImage.push({
                                    result_image : `${IMAGE_URL}/image?path=/project/${project_id}/stage6/${res.data.data.files[i]}&width=1440`,
                                    no : SpanNum
                                })
        ...
        for (let i = 1; i < spanCount+1; i++) {
            ...
            <TabPane tab={"Span" + `${i}`} key={i}>
                <div className={styles.DetectorDiv} >
                    <span className={styles.DetectorTitle}>{resultOri === true ?  ImgListOri[num].original_image.substring(ImgListOri[num].original_image.indexOf("/S") + 6, ImgListOri[num].original_image.indexOf(".png") + 4) : ""}</span>
                    <Button className={styles.downloadBtn} onClick={downloadCouple}>{t.coupledownload}</Button> 
                    <Button className={styles.downloadBtn} onClick={openWindow}>균열 결과 수정</Button>
    ...
    async function download(){
        ...
        let src = `${IMAGE_URL}/image?path=/project/${project_id}/stage6_Couple/${ImageUrlCutWidthName}`;
    ...
    <div>
        <div className={styles.DivArea}>
            <div>
                <p className={styles.subOrder}>{t.cnCrackDetectorMeasure} &gt; {t.cnCrackDetector}</p>
