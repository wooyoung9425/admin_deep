...
export default function BridgeCrackMeasureSetting() {
  ...
  const CrackMeasureList : MeasureList[] = [];

  const [ImgList, setImgList] = useState<any | undefined>([]); 
  const [NewCrackMeasureList, setNewCrackMeasureList] = useState<any[]>(CrackMeasureList);
  ...
  useEffect(()=>{
            ...
            for(let i = 0; i < settings.span_count; i++){
                for(let k = 0; k < settings.span_length/settings.span_class_length; k++){
                  CrackMeasureList.push({
                        ...
                        crack_image_folder : `stage6_crack/${BridgeType}/S${settings.span_number_list[i].num.length === 2 ? settings.span_number_list[i].num+"0"+(k+1) : "0"+settings.span_number_list[i].num+"0"+(k+1)}`,
                        output_folder: `stage7_crack/${BridgeType}`,
                    })
                }
            }
        }
        ...
        if(BridgeType === "Pier" ){
            for(let i = 0; i<settings.pier_count; i++){
                CrackMeasureList.push({
                    ...
                    crack_image_folder : `stage6_crack/${BridgeType}/P${settings.pier_number_list[i].num.length === 2 ? settings.pier_number_list[i].num : "0"+settings.pier_number_list[i].num}`,
                    output_folder: `stage7_crack/${BridgeType}`,
                })
            }
        }
    ...
    setNewCrackMeasureList(CrackMeasureList)
}, [BridgeType])
  ...
  const onChange = (e: RadioChangeEvent) => {
    // let copyArrDetectorList = [...NewCrackMeasureList];
    ...
    axios({
        ...
        if(e.target.value===1){
            ...
            //     setNewCrackMeasureList(copyArrDetectorList)
            // }
        }
    }).catch((err) => {
        console.log(err);
  ...
  const MeasureStart = () => {
    alert("교량 균열 측정 작업이 시작되었습니다. 잠시만 기다려주세요.")
        setTimeout(function() {
            alert("균열 측정이 완료되었습니다.")
        }, 2000);
}

  const Test = () => {
    console.log(NewCrackMeasureList)
  }
  ...
  return (
    <div>
      <div style={{width: "50em", margin: "0 auto"}}>
        <br />
        <p style={{fontSize:"25px"}}>{t.cnMeasure}</p>
        ...
        <Form labelCol={{span: 0, }} wrapperCol={{span: 10, }} layout="horizontal">
          <Form.Item label={t.cnChoiceModel}>
            <Select defaultValue="1" onChange={onChangeModel}>
              ...
              <Select.Option value="2" disabled>Crack Measure TF2.x</Select.Option>
            </Select>
          </Form.Item>
        </Form>
      ...
      <div style={{width:"50em", margin:"0 auto"}}>
        ...
        <p style={{fontSize:"20px", fontWeight:"bold"}}>{t.cnMeasureList}</p>
        <div>
