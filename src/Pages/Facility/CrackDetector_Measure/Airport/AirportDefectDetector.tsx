...
export default function AirportDefectDetector() {
            ...
            headers : {"accept" : `application/json`, "access-token" : `${token}`}, 
                ...
                if (res.data.check === true) {
                    ...
                    axios({
                        ...
                        headers: { "accept": `application/json`, "access-token": `${token}` },
                        params: {
                            path: `/project/${project_id}/stage6_defect`
                        }
                    }).then((res)=>{
                        if (res.data.check === true) {
                            // console.log("성공")
                            ...
                            for(let i = 0; i<res.data.data.files.length; i++){
                                ...
                                const SpanNum = res.data.data.files[i].substring(indexFront, indexBack-7)
                                // console.log(SpanNum)
                                if(res.data.data.files[i].includes(".png")){
                                    ResultImage.push({
                                        result_image : `${IMAGE_URL}/image?path=/project/${project_id}/stage6_defect/${res.data.data.files[i]}&width=1440`,
                                        no : SpanNum
                                    })
    ...
    const rendering = () => {
        ...
        resultCrack === true ? localStorage.setItem('edit_defect_image', ImgListCrack[num].result_image) : localStorage.setItem('edit_defect_image', "")
